...
# This vector represents the acceleration due to gravity in 3D space
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    # creating 3*3 identity matrix 'I'
    I = np.identity(3)
    # the scaled covariance matrix R
    R = I * sensor_var
    # Kalman Gain
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    for i in range(len(gnss.t)):
        if gnss.t[i] == imu_f.t[k - 1]:
            p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_gnss, p_cov[k], gnss.data[i].T, p_est[k],
                                                                        v_est[k], q_est[k])
    for i in range(len(lidar.t)):
        if lidar.t[i] == imu_f.t[k - 1]:
            p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_lidar, p_cov[k], lidar.data[i].T, p_est[k],
                                                                        v_est[k], q_est[k])
    for i in range(len(gnss.t)):
        if gnss.t[i] == imu_f.t[k - 1]:
            p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_gnss, p_cov[k], gnss.data[i].T, p_est[k],
